...
再**按本次任务需要**有选择地读：`modules.md` 里相关模块的 `## 小节`（已拆分的读 `modules/<模块>.md`）、`interface-map.md`、`database-map.md`、`risk-points.md`、近期 `task-log.md`。不要一次全量读完，按任务相关性取。

**顺着 `[[名称]]` 链接扩展**：记忆里的 `[[某模块]]`/`[[某表]]` 是关系标注（指向 `modules.md` / `database-map.md` 的对应小节，或已拆分的独立文件）。本次任务触达哪个实体，就顺链接读它那一节——这是"按需精准扩展"的路径，代替全量加载；任务中途碰到没预载的 `[[实体]]`，先读它再动手。
...
### 3. 已加载记忆
- overview.md：是 / 否
- 另读了：modules.md 的 ## xxx 小节、database-map…… / 无
