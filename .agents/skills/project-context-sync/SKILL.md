...
1. **首次浏览/改动前，必须先在「目标项目根」同步远程。** 目标项目 = **本次要读/改的文件所在的仓库**——不是会话主目录、不是环境信息里报告的当前目录；工作区常是"一个父目录装着多个独立仓库"（前后端分仓、多项目并存），要碰几个仓库就逐个检查几个。**"主目录不是 git 仓库"不能推断"目标项目不是仓库"**——必须到目标文件路径向上实际验证过，才允许跳过；确认不是仓库才跳过，不报错、不强行 init。
...
**0a. 先定位目标项目根**——以「本次要读/改的文件路径」向上找仓库根，绝不用会话主目录的状态来推断：

```bash
# 在目标文件所在目录执行（-C 指到要改的目录），拿到的才是目标项目的仓库根
git -C <要读改的文件所在目录> rev-parse --show-toplevel 2>/dev/null || echo "NOT_A_GIT_REPO"
```

```powershell
# Windows PowerShell
git -C <要读改的文件所在目录> rev-parse --show-toplevel 2>$null; if ($LASTEXITCODE -ne 0) { "NOT_A_GIT_REPO" }
```

- 本次任务要碰几个仓库，就做几遍（前后端分仓时 service 与 backweb **各自**都要同步）。
- 确认目标项目根**不是** git 仓库 → 跳过同步，声明里写「<目录>：非 git 仓库，跳过」，进第 1 步。

**0b. 对每个目标仓库**，先看工作区是否干净，再决定怎么拉：
...
再**按本次任务需要**有选择地读：`details.md` 里相关实体的 `## 小节`（模块/表/接口都在这一个文件里；已拆分的读对应独立文件）、近期 `task-log.md`。不要一次全量读完，按任务相关性取。

**顺着 `[[名称]]` 链接扩展**：记忆里的 `[[某模块]]`/`[[某表]]` 是关系标注（统一指向 `details.md` 的对应小节，或已拆分的独立文件）。本次任务触达哪个实体，就顺链接读它那一节——这是"按需精准扩展"的路径，代替全量加载；任务中途碰到没预载的 `[[实体]]`，先读它再动手。

（向后兼容：只有 `docs/AI开发交接记录.md` 的老项目，读它当 task-log。）

若 `docs/ai-memory/` 或 `overview.md` 不存在，如实声明「项目暂无可加载的共享记忆」，不为完成流程而创建空文件。项目计划、需求、业务流程、字段/内容契约和验收依据统一从 `docs/` 按任务相关性读取；不要去根目录寻找或新建业务文档，旧项目已有根目录业务文档时只兼容读取、不自动迁移。
...
上面的反馈**不是可选项**。每次任务开工的**第一条回复**必须包含一段简短「开工声明」，最简 2-3 行即可。**必须逐仓库点名**（写出仓库目录名），防止「检查了 A 仓库、改的却是 B 仓库」这种错位不可见：

```text
开工声明：目标仓库 union_service——已 pull 快进；backweb——已 pull（或：跳过，原因）；
...
### 坑 6：用会话主目录的状态推断目标项目（真实事故）
环境信息说"当前工作目录不是 git 仓库"，就以为整个任务不用 pull——但要改的文件在另一个目录，那是个**独立仓库**，远程还躺着同事几天前的提交、且动了同一个文件。结果基于过时代码改完，用户提交时被迫产生 merge，险些回退同事的修复。教训：**同步检查必须以「要改的文件路径」向上定位仓库根来做（0a 步），逐仓库执行，并在开工声明里逐仓库点名**——"主目录不是仓库"永远不能推出"目标项目不是仓库"。
