...
> 🚀 **想让你的 AI 学会并永久记住这套规则?** 把这句发给它:「读这个仓库的 `.agents/skills/onboard-aiteamops/SKILL.md` 并照做,把规则接入你的永久记忆」。详见 [ONBOARDING.md](ONBOARDING.md)。
> （注意:模型本身没有永久记忆,持久化靠把规则写进宿主工具每次加载的文件——`onboard-aiteamops` 会在你确认后帮你落盘,这是一次安装动作,不是读一遍就自动记住。）
...
docs       →  管「业务事实」和「验收依据」：计划、需求、流程、契约、验收、项目记忆
```

三者职责不重叠：**规则不写进 Skill，业务事实不写进 AGENTS.md。** 本仓库聚焦中间这一层 —— Skills。

项目结构遵循「**最小默认骨架、其余按需生长**」：根目录只放 AI 工具入口与工程必要文件；计划、需求、业务流程、字段/内容契约和验收依据统一放进 `docs/`；没有真实内容就不预建文件或目录。业务契约不使用 `Agent` 命名，`AGENTS.md` 只保留规则与路由。
...
| Skill | 触发场景 | 一句话作用 |
|---|---|---|
| [`onboard-aiteamops`](.agents/skills/onboard-aiteamops/SKILL.md) | 「学习这个仓库并存到永久记忆」「接入 AITeamOps」，或把 ONBOARDING.md 指给某个 AI | 自举：教一个新工具/模型读懂框架→写进它的永久记忆文件→自检，让规则在该用户所有项目持久生效 |
| [`team-ai-workspace-bootstrap`](.agents/skills/team-ai-workspace-bootstrap/SKILL.md) | 「初始化下项目」、新项目接入 AI 规范、统一 AGENTS.md | **分析 + 按需预填**：生成含真实内容的 AGENTS.md 与项目记忆；不创建空骨架、空业务文档或重复入口 |
...
| [`requirement-delivery-flow`](.agents/skills/requirement-delivery-flow/SKILL.md) | 收到明确开发需求、要动手实现时；或用户说「别老问我」「一次性做完」「别绕弯子」 | **治「空转打转」**：想清楚（结合代码/影响/角色/复用旧代码）→ 最多确认一次 → 一口气实现到完成不空转播报 → 完成后按 JDK 版本静态自检（干活侧） |
...
| [`ai-deliverable-review`](.agents/skills/ai-deliverable-review/SKILL.md) | 「复核下 Codex 做的这个功能」「全面检查这个新模块」「验收这块代码」 | 验收级复核另一个 AI/同事的交付：不信总结只信代码，权限/SQL/字段/并发逐层核对，声明验证等级（L0-L3） |
| [`cross-project-experience`](.agents/skills/cross-project-experience/SKILL.md) | 「这条经验所有项目都用得上」「记到全局」「以后所有项目都这样」 | 把**对任何项目都适用**的经验沉淀进全局指令文件（CLAUDE.md/AGENTS.md），微型的永久经验累积，改全局前需确认 |
| [`multi-tool-entrypoint-sync`](.agents/skills/multi-tool-entrypoint-sync/SKILL.md) | 「让 Codex/Claude/Cursor 行为一致」「多模型不一致」「配置各工具规则文件」 | 以 AGENTS.md 为唯一真源，只为明确指定的实际工具生成薄入口，一源多入口 |
...
| [`mysql-readonly-probe-via-java`](.agents/skills/mysql-readonly-probe-via-java/SKILL.md) | 机器无 mysql 客户端、需确认库结构/数据；写 SQL 前/改接口后主动核验字段格式 | 用 JDK + 现成 connector jar 做 MySQL **只读**探测与数据格式核验，绝不写库 |
| [`mysql-guarded-write`](.agents/skills/mysql-guarded-write/SKILL.md) | 用户明确说「你直接执行这个 SQL」「允许你写库」 | **仅限明确授权**的受控写库：表白名单过目、逐条预检+报告影响行数、先备份可回滚，DROP/TRUNCATE/无 WHERE 更新绝对禁止 |

每个 Skill 都内置「红线」与「认知边界」，明确什么动作必须停下来交回给人（push、写库、改业务逻辑等）。

> **读写闭环**：`project-context-sync`（开工读：同步远程 + 加载记忆）→ 干活（`requirement-delivery-flow`：想清楚→最多确认一次→一口气做完→自检，治「空转打转」）→`ai-handoff-doc-update`（收尾写：沉淀回记忆）。一个人用多模型、或一个团队多人协作时，所有 AI 都从同一份共享上下文出发、又写回同一处，理解持续收敛而不漂移。多工具一致性则由 `multi-tool-entrypoint-sync` 把各工具入口统一指向 `AGENTS.md` 解决。
>
> **三层记忆**：① 项目专属事实 → `docs/ai-memory/`（`ai-handoff-doc-update`，随项目共享）；② 跨项目通用经验 → 全局指令文件（`cross-project-experience`，对所有项目生效，改前确认）；③ 一次性失误 → 不记。第 ② 层让 AI 跨项目地累积经验、越用越强——一个微型的永久经验累积过程。
...
AITeamOps/
├── README.md
├── ONBOARDING.md                                    # 让一个新 AI 学会并永久记住本框架的冷启动入口
├── AGENTS.md                                        # 本仓库自身的 AI 协作规则（框架活样板）
├── CLAUDE.md                                        # Claude Code 薄入口：仅 @AGENTS.md
...
├── templates/
│   └── 需求说明模板.md                               # 给 AI 的开发任务书模板（输入侧治理）
├── scripts/
│   ├── install-windows.ps1                          # Windows 下只安装用户级 Skills
│   ├── sync-tool-entrypoints.ps1                    # 按明确工具清单生成薄入口
│   └── validate.sh                                  # 校验脚本（本地/CI 通用）
└── .agents/
    └── skills/
        ├── onboard-aiteamops/SKILL.md               # 自举：教新模型学会并永久记住本框架
        ├── team-ai-workspace-bootstrap/SKILL.md     # 环境初始化
        ├── project-context-sync/SKILL.md            # 开工前同步远程 + 加载记忆（读侧，含开工声明）
        ├── requirement-delivery-flow/SKILL.md       # 干活阶段执行纪律：想清楚→最多确认一次→一口气做完→自检，治空转
        ├── ai-handoff-doc-update/SKILL.md           # 任务后文档沉淀（项目记忆，写侧，含收尾沉淀结论）
        ├── ai-deliverable-review/SKILL.md           # 验收级复核 AI/他人交付的模块
        ├── cross-project-experience/SKILL.md        # 跨项目经验沉淀到全局（全局累积）
        ├── multi-tool-entrypoint-sync/SKILL.md      # 多工具入口统一指向 AGENTS.md
        ├── git-commit-guard/SKILL.md                # 安全提交关卡
        ├── mysql-readonly-probe-via-java/SKILL.md   # 数据库只读探测 + 字段格式核验
        └── mysql-guarded-write/SKILL.md             # 授权下的受控写库
```
...
```powershell
# Windows PowerShell（或直接跑 .\scripts\install-windows.ps1，只安装用户级 Skills）
git clone https://github.com/kross88/AITeamOps.git
...
>
> **框架更新后同步**：本仓库更新时，各成员 `git pull` 本仓库后重跑一次安装（重新复制 skills），否则本地装的还是旧版；也可直接让 AI 代劳「同步一下 AITeamOps 到本地」。

**项目级（按需固定版本或随团队仓库分发）**

默认先安装到用户级 `~/.agents/skills/`。只有项目需要固定 Skill 版本或希望随 Git 分发时，才把选定的 Skill 放进项目 `.agents/skills/`；不要创建空目录。项目规则和记忆请让 AI 使用 `team-ai-workspace-bootstrap` 分析后生成，机械安装脚本不再创建空项目骨架。
