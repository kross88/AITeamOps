...
| [`ai-handoff-doc-update`](.agents/skills/ai-handoff-doc-update/SKILL.md) | 任务完成后「沉淀一下」「更新文档/AGENTS.md」「写交接记录」 | 把本次产出按类别路由到 docs/ai-memory，只在稳定索引变化时才精修 AGENTS.md，不堆任务流水 |
...
└── .agents/
    └── skills/
        ...
        ├── ai-handoff-doc-update/SKILL.md           # 任务后文档沉淀
...
`ai-handoff-doc-update` 的目标是「每次完成任务都沉淀项目记忆」。但要明确一个边界：**Skill 是靠 `description` 匹配的自愿触发，不是 100% 强制。** 想让它稳定执行，按强度从低到高有三种做法：

**1. 规则驱动（推荐，贴合 AGENTS.md 体系）**
...
**2. Hook 强制（harness 执行，硬保证）**

在 AI 工具的配置（如 Claude Code 的 `settings.json`）里加一个 `Stop` hook，每次回复结束由 harness 注入提醒，强制 AI 检查是否需要沉淀。

优点：真正「每次都触发」。局限：较"吵"，且无法智能区分「刚完成真任务」和「只是答了个问题」，需要 Skill 内部的「触发时机」判断来过滤。

**3. 两者结合（最稳）**

规则定义「该做什么、何时做」，Hook 兜底「别忘了做」。Skill 正文里的「触发时机」一节负责过滤掉纯问答，避免 Hook 噪音。

> 不同工具的 hook 机制不同，且修改配置文件属于环境变更，请按自己工具的文档手动配置，或在明确授权后让 AI 代为修改。
